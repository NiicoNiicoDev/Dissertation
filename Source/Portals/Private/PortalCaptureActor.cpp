...
FMatrix APortalCaptureActor::GetLeftEyeProjection() const
{
	...
	FMatrix Result = Stereo->GetStereoProjectionMatrix(EStereoscopicPass::eSSP_LEFT_EYE);
	return Result;
}
...
{
	TSharedPtr< IStereoRendering, ESPMode::ThreadSafe > Stereo = GEngine->XRSystem->GetStereoRenderingDevice();
	FMatrix Result = Stereo->GetStereoProjectionMatrix(EStereoscopicPass::eSSP_RIGHT_EYE);
	return Result;
}
...
{
	GEngine->StereoRenderingDevice->CalculateStereoViewOffset(EStereoscopicPass::eSSP_LEFT_EYE, InRotation, GetWorld()->GetWorldSettings()->WorldToMeters, InPosition);
...
{
	GEngine->StereoRenderingDevice->CalculateStereoViewOffset(EStereoscopicPass::eSSP_RIGHT_EYE, InRotation, GetWorld()->GetWorldSettings()->WorldToMeters, InPosition);
	OutRotation = InRotation;
	OutPosition = InPosition;
}
...
{
	GEngine->StereoRenderingDevice->CalculateStereoViewOffset(EStereoscopicPass::eSSP_LEFT_EYE, ViewInfo.Rotation, GetWorld()->GetWorldSettings()->WorldToMeters, ViewInfo.Location);
	return ViewInfo;
}

FMinimalViewInfo APortalCaptureActor::AdjustRightEyeViewInfo(FMinimalViewInfo ViewInfo) const
{
	GEngine->StereoRenderingDevice->CalculateStereoViewOffset(EStereoscopicPass::eSSP_RIGHT_EYE, ViewInfo.Rotation, GetWorld()->GetWorldSettings()->WorldToMeters, ViewInfo.Location);
	return ViewInfo;
}
...
{
	checkf(fabsf(MinLocalPoint.Z - MaxLocalPoint.Z) < 1e-5f, TEXT("The portal should be a plane with normal (0, 0, 1)"));
